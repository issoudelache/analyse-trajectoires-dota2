...
Ce rapport détaillera l'intégralité de notre démarche. Après avoir exposé les objectifs et la problématique du projet (Section 2), nous présenterons l'architecture de notre chaîne de traitement (Section 3) ainsi que les éléments techniques et algorithmiques qui la composent (Section 4). Nous consacrerons ensuite une large partie à l'évaluation expérimentale de nos modèles, de la compression géométrique (Section 5) au clustering spatial (Section 6), jusqu'à l'extraction finale des motifs stratégiques (Section 7). Enfin, nous conclurons sur les apports de ce projet et ses perspectives d'évolution (Section 8).

% ==========================================
% SECTION 2 : OBJECTIFS ET PROBLÉMATIQUE
% ==========================================
...
\subsection{État de l'art et Travaux existants}
...
% ==========================================
% SECTION 3 : ARCHITECTURE LOGICIELLE ET FONCTIONNALITÉS
% ==========================================
\section{Architecture Logicielle et Fonctionnalités}
...
\subsection{Le Pipeline de Traitement Stratégique}
...
\subsection{Fonctionnalités de l'Application : Dota 2 Trajectory Analyzer}
...
\begin{itemize}
    ...
    \item \textbf{Comparateur Synchronisé (Brut vs Compressé) :} Module d'inspection visuelle affichant deux cartes côte à côte (Figure \ref{fig:gui_comparison}). Le système pré-calcule l'intégralité des \textit{frames} en mémoire pour garantir une lecture temporelle parfaitement fluide et synchronisée.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Orchestrateur End-to-End (Pipeline) :} Module automatisant la chaîne de valeur (Figure \ref{fig:gui_pipeline}). L'exécution parallèle s'appuie sur le \textit{multiprocessing} natif ($\text{CPU\_count} - 2$ cœurs). Des indicateurs visuels suivent l'avancement des étapes jusqu'à la génération des résultats analytiques finaux.
\end{itemize}
...
% ==========================================
% SECTION 4 : ÉLÉMENTS TECHNIQUES ET STRUCTURES DE DONNÉES
% ==========================================
\section{Éléments Techniques et Structures de Données}
...
\subsection{Dépendances et Implémentations \textit{From Scratch}}
...
\subsection{Structures de Données et Optimisation Mémoire}
L'ingestion de millions de points a nécessité une conception rigoureuse des structures de données pour éviter la saturation de la RAM. Pour illustrer la transformation opérée par notre pipeline, la Figure \ref{fig:data_transition} présente la transition entre la donnée brute et la donnée structurée :
...
\subsection{Pseudo-code de l'algorithme central : MDL Compress}
...
% ==========================================
% SECTION 5 : EXPERIMENTATIONS MDL
% ==========================================
\section{Expérimentations 1 : Validation de la Compression par Seuil (MDL)}
...
\textbf{Jeu de données utilisé :} Nos expérimentations portent sur un corpus de 70 matchs réels de \textit{DotA 2} (fichiers CSV), représentant 10 joueurs par match. Après compression avec $w_{error}=12$, ce corpus génère environ 18\,000 segments. Pour l'étape de fouille de motifs (Section 7), un sous-ensemble de 30 matchs est utilisé, produisant 1\,174 séquences fréquentes avec un support minimum de $15\%$.
...
\subsection{Invariance au Bruit et Stabilité Structurelle}
...
\subsection{Validation Visuelle de la Robustesse}
\begin{figure}[H]
  \centering
...
\subsection{Évaluation comparative avec les méthodes standards (Baseline)}
...
\subsubsection{Validation qualitative et quantitative}
\begin{figure}[H]
  \centering
...
\subsection{Évaluation de la Complexité Computationnelle}
\begin{figure}[H]
  \centering
...
\subsection{Analyse de Sensibilité : Justification du paramètre $w_{error}$}
...
\subsection{Robustesse au Bruit Gaussien}
...
% ==========================================
% SECTION 6 : EXPERIMENTATIONS CLUSTERING
% ==========================================
\section{Expérimentations 2 : Évaluation des Algorithmes de Clustering}

\subsection{Optimisation de $k$ : Compromis entre Topologie et Sémantique}
...
\subsection{Évaluation Topologique Comparative des Algorithmes}
...
\subsection{Analyse de Sensibilité et Robustesse de la Propagation d'Affinité (AP)}
...
\subsection{Scalabilité vs Qualité Sémantique : Le choix final}
...
% ==========================================
% SECTION 7 : EXPERIMENTATIONS PREFIXSPAN
% ==========================================
\section{Fouille de Motifs Séquentiels et Interprétation Stratégique}

\subsection{Gestion de la volumétrie et malédiction des séquences longues}
...
\subsection{Analyse des Motifs Séquentiels (PrefixSpan) et Graphe de Markov}
...
% ==========================================
% SECTION 8 : RÉCAPITULATIF DES HYPERPARAMÈTRES
% ==========================================
\section{Récapitulatif des Hyperparamètres du Pipeline}
...
\midrule
$w_{error}$ & 12.0 & Tolérance MDL (distance perp.) & Sec.~5.6 : plateau compression 96\% \\
$k$ & 10 & Nombre de macro-zones & Sec.~6.1 : coude Silhouette + richesse PrefixSpan \\
$N_{AP}$ & 3\,000 & Sous-échantillon pour AP & Sec.~6.3 : seuil de stabilité topologique \\
Algo. clustering & Affinity Prop. & Découpage spatial & Sec.~6.4 : absence de doublons géographiques \\
$min\_support$ & 15\% & Seuil PrefixSpan & Sec.~7.1 : filtrage des motifs triviaux \\
$max\_length$ & 5 & Profondeur max. des motifs & Sec.~7.1 : limitation de l'explosion combinatoire \\
$damping$ (AP) & 0.7 & Amortissement message-passing & Sec.~6.3 : stabilité de convergence \\
$preference$ (AP) & $-5\,000$ & Auto-sélection des exemplaires & Sec.~6.3 : convergence vers $\sim$10 clusters \\
$min\_length$ & 5.0 & Longueur min. des segments & Filtrage des micro-mouvements \\
Nb. de matchs & 70 (compr.) & Corpus total & ---  \\
 & 30 (fouille) & Sous-ensemble PrefixSpan & Sec.~7.1 \\
\bottomrule
\end{tabularx}
\end{table}
...
\vspace{0.5cm}
\textbf{Perspectives :} L'essence même d'un MOBA réside dans la coordination d'équipe et l'asymétrie des camps. Plusieurs axes d'amélioration se dessinent :
\begin{itemize}
