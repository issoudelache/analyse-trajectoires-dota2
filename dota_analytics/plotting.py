"""
Visualisation et chargement des données compressées.

Sections :
  - Constantes (PLAYER_COLORS, load_canvas)
  - Chargement I/O (get_available_w_errors, get_available_games, load_compressed_data)
  - Overlay statique (generate_static_overlay)
  - Comparaison original vs compressé (generate_comparison_image)
  - Overlay interactif (InteractiveOverlay)
  - Clusters (plot_cluster_on_map, plot_markov_network)
"""
...
def load_canvas(canvas_path) -> np.ndarray:
    """Charge la carte Dota 2 et la recadre en carré (partie centrale)."""
    canvas_full = mpimg.imread(canvas_path)
    height, width = canvas_full.shape[:2]
    if width > height:
        left = (width - height) // 2
        return canvas_full[:, left : left + height]
    return canvas_full
...
def get_available_games(data_dir, w_error):
    ...
    w_error_dir = Path(data_dir) / f"w_error_{w_error_str}"
...
def generate_comparison_image(csv_path, json_path, w_error, output_dir):
    ...
    try:
        ...
        ax2.set_title(
            f"Compressé: {total_segments} segments ({reduction:.1f}% réduction)",
            fontsize=14,
            fontweight="bold",
        )
...
class InteractiveOverlay:
    ...
    def __init__(self, canvas_path, data_dir, w_error, game_id):
        ...
        self.canvas = load_canvas(canvas_path)
...
def plot_cluster_on_map(canvas_path, clusters_file, compressed_dir_path, cluster_id):
    """Visualise les segments d'un cluster sur la carte du jeu (mode fenêtre)."""
    ...
    if not segments_locations:
        print(f"Aucun segment trouvé pour le Cluster {cluster_id} !")
        return
...
def plot_markov_network(patterns_dict: dict, min_len: int = 2, output_path: str = None):
    ...
    if len(G.nodes) == 0:
        print("Aucun motif multi-étapes à tracer.")
        return
    ...
    node_sizes = [
        min(
            3000,
            300
            + 50 * G.in_degree(n, weight="weight")
    ...
    plt.title(
        "Graphe des Transitions Macroscopiques (Motifs Dota 2)",
        fontsize=18,
        fontweight="bold",
    )
    plt.axis("off")
