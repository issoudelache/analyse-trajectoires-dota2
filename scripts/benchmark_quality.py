...
def benchmark_quality(
    data_dir: Path, output_dir: Path, num_matches: int = 50, w_error: float = 12.0
):
    ...
    if len(csv_files) < num_matches:
        print(
            f"⚠️  Seulement {len(csv_files)} fichiers disponibles (demandé: {num_matches})"
        )
...
def generate_quality_plot(df: pd.DataFrame, output_dir: Path, w_error: float):
    ...
    bp = ax4.boxplot(
        box_data,
        labels=["RMSE", "Erreur Moyenne", "Erreur Max"],
        patch_artist=True,
        showmeans=True,
    )
