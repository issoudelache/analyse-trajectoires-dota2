...
def generate_triptych(w_error, sigma):
    ...
    ax2.set_title(f'2. Injection de Bruit (σ={sigma})', 
    ...
    ax3.plot(x_noisy, y_noisy, 
            color='lightgray', linewidth=0.6, alpha=0.4, linestyle='-',
            zorder=1)
    ax3.plot(x_noisy, y_noisy, 
            color='lightgray', linewidth=0, alpha=0.35,
            marker='x', markersize=4, markeredgewidth=0.8,
            label='Données bruitées', zorder=2)
    ...
    for i, segment in enumerate(segments):
        ...
        # Ligne
        ax3.plot(x_seg, y_seg, 
                color='red', linewidth=2.5, alpha=0.9, zorder=10)
        
        # Points aux jonctions
        ax3.plot(x_seg, y_seg, 'o',
                color='red', markersize=8,
                markeredgecolor='darkred', markeredgewidth=1.5, zorder=11)
    
    ax3.set_title(f'3. Résultat MDL (Filtrage w={w_error})', 
...
if __name__ == '__main__':
    ...
    parser.add_argument('--w_error', type=float, default=12.0,
