...
class Segment:
    ...
    def vector(self) -> np.ndarray:
        ...
        return np.array(
            [self.end.x - self.start.x, self.end.y - self.start.y], dtype=np.float64
        )
...
class JSONExporter:
    ...
    def export_match(
        ...
        output_path: Path,
        w_error: float = 10.0,
        original_points: Dict[int, int] = None,
    ) -> Path:
        ...
        for player_id in sorted(results.keys()):
            ...
            num_original = (
                int(original_points.get(player_id, 0)) if original_points else 0
            )
    ...
    @staticmethod
    def export_batch(
        matches_results: Dict[str, Dict[int, List[Segment]]],
        output_dir: Path,
        w_error: float = 10.0,
        original_points_per_match: Dict[str, Dict[int, int]] = None,
    ) -> List[Path]:
...
def export_match(
    ...
    Returns:
        Path du fichier créé
    """
    return JSONExporter.export_match(
        results, match_id, output_path, w_error, original_points
    )
