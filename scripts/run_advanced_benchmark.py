...
def get_random_samples(
    ...
    if len(trajectory.points) < duration_ticks:
        raise ValueError(
            f"Trajectoire trop courte: {len(trajectory.points)} < {duration_ticks}"
        )
    ...
    for i in range(n_samples):
        ...
        sample_points = [
            p for p in trajectory.points if start_tick <= p.tick < end_tick
        ]
        ...
        if len(sample_points) >= 10:  # Minimum 10 points pour être valide
            ...
            print(
                f"⚠️ Échantillon {i + 1} invalide ({len(sample_points)} points), réessai..."
            )
    ...
    for orig_point in original_points:
        if orig_point.tick in segment_map:
            ...
            if t2 == t1:
                ...
                reconstructed_x = segment.start.x + alpha * (
                    segment.end.x - segment.start.x
                )
                reconstructed_y = segment.start.y + alpha * (
                    segment.end.y - segment.start.y
                )

            error = np.sqrt(
                (orig_point.x - reconstructed_x) ** 2
                + (orig_point.y - reconstructed_y) ** 2
            )
...
def run_single_test(
    ...
    segment_stability = calculate_segment_stability(
        nb_segments_clean, nb_segments_noisy
    )
...
def run_benchmark(samples: List[Trajectory]) -> pd.DataFrame:
    ...
    print(
        f"   Configurations: {len(W_ERROR_VALUES)} w_error × {len(SIGMA_VALUES)} sigma × {len(samples)} échantillons"
    )
    print()
        ...
        for sigma in SIGMA_VALUES:
            for sample_id, sample in enumerate(samples):
                ...
                if completed % 10 == 0 or completed == total_tests:
                    ...
                    print(
                        f"   [{completed}/{total_tests}] {progress:.1f}% | w={w} σ={sigma} sample={sample_id} | RMSE={result.rmse_clean:.2f} Stab={result.segment_stability:.1f}%"
                    )
...
def generate_heatmaps(df_agg: pd.DataFrame):
    ...
    pivot_segments = df_agg.pivot(
        index="w_error", columns="sigma", values="nb_segments_noisy_mean"
    )

    pivot_rmse = df_agg.pivot(
        index="w_error", columns="sigma", values="rmse_clean_mean"
    )
    ...
    ax.set_title(
        "Heatmap 1 : Stabilité Structurelle\n(Nombre de segments moyen)",
        fontsize=14,
        fontweight="bold",
        pad=20,
    )
    ...
    for i in range(len(pivot_segments.index)):
        for j in range(len(pivot_segments.columns)):
            ...
            if not np.isnan(value):
                text_color = (
                    "white" if value > pivot_segments.values.max() * 0.6 else "black"
                )
                ax.text(
                    j,
                    i,
    ...
    ax.set_ylabel("w_error (Sensibilité MDL)", fontsize=12, fontweight="bold")
    ax.set_title(
        "Heatmap 2 : Fidélité au Signal Original\n(RMSE Clean - Métrique de Débruitage)",
        fontsize=14,
        fontweight="bold",
        pad=20,
    )
    ...
    for i in range(len(pivot_rmse.index)):
        for j in range(len(pivot_rmse.columns)):
            ...
            if not np.isnan(value):
                ...
                text_color = (
                    "white" if value > pivot_rmse.values.max() * 0.6 else "black"
                )
                ax.text(
                    j,
                    i,
...
def main():
    ...
    print(
        f"   Tick range: {full_trajectory.points[0].tick} → {full_trajectory.points[-1].tick}"
    )
    print()
    ...
    print("=" * 80)
    print(
        f"RMSE Clean moyen: {df_agg['rmse_clean_mean'].mean():.2f} ± {df_agg['rmse_clean_std'].mean():.2f}"
    )
    print(
        f"Stabilité moyenne: {df_agg['segment_stability_mean'].mean():.1f}% ± {df_agg['segment_stability_std'].mean():.1f}%"
    )
    print(
        f"Segments (noisy) moyen: {df_agg['nb_segments_noisy_mean'].mean():.1f} ± {df_agg['nb_segments_noisy_std'].mean():.1f}"
    )
    print()
    ...
    print("   - heatmap_stabilite.png : Nombre de segments (w × sigma)")
    print("   - heatmap_fidelite.png : RMSE Clean (w × sigma)")
    print()
