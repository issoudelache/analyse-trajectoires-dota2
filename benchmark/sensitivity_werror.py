...
def _compute_safe_workers(
    max_seg: int, max_ram_gb: float, n_traj_bytes: int = 0
) -> int:
...
def evaluate_kmedoids(
    sim_matrix, k, seed, X_scaled, n_seg, sil_sample=SILHOUETTE_SAMPLE
):
...
def _worker_single_werror(args_tuple):
    ...
    if n_seg_total < 20:
        with lock:
            counter.value += 1
            print(
                f"  [{counter.value:3d}/{total}] w={w_error:7.2f}  →  "
                f"{n_seg_total:5d} seg   SKIP (< 20)",
                flush=True,
            )
        return [
            _empty_row(
                w_error, n_raw, n_seg_total, n_seg_total, comp_rate, t_compress, algo
            )
    ...
    with lock:
        counter.value += 1
        print(
            f"  [{counter.value:3d}/{total}] w={w_error:7.2f}  →  {n_seg:5d} seg | "
...
def run_benchmark(
    ...
    *,
    ...
    n_traj_bytes = (
        _estimate_traj_bytes(trajectories) if platform.system() == "Windows" else 0
    )
    ...
    if n_workers > 1 and n_workers > safe_workers:
        print(
            f"⚠️  {n_workers} workers × {per_worker_gb:.1f} Go/worker "
    ...
    print(
        f"Benchmark : {total} w_error × (KMeans + KMedoids) × {n_seeds} graines + AP"
        f" = {total_runs} runs",
        flush=True,
    )
    print(
        f"K fixe    : {k}  |  Sous-éch. TRACLUS : {max_seg}  |  Sil. sample : {sil_sample}",
        flush=True,
    )
    print(f"Mode      : {mode}", flush=True)
    print(
        f"RAM estimée : {per_worker_gb:.1f} Go/worker × {n_workers} "
...
def _run_sequential(
    ...
    *,
    ...
    for idx, w_error in enumerate(w_errors):
        ...
        print(
            f"\r  ⏳ [{idx + 1:3d}/{total}] w={w_error:7.2f}  en cours…",
            end="",
            flush=True,
        )
...
def _empty_row(w_error, n_raw, n_seg_total, n_seg, comp_rate, t_compress, algorithm):
...
Exemples :
  ...
  python benchmark/sensitivity_werror.py --max_files 30 --n_seeds 10
    ...
    print(f"Grille : {len(w_errors)} valeurs  ({w_errors[0]:.1f} → {w_errors[-1]:.1f})")
