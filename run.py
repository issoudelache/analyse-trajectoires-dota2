...
def cmd_visualize(args):
    ...
    if not json_path.exists():
        ...
        print(
            f"💡 Lancez d'abord: python run.py compress --w_error {w_error} --match_id {match_id}"
        )
        return
    ...
    success, mid, size, *error = generate_comparison_image(
        csv_path, json_path, w_error, output_dir
    )
...
def cmd_zoom_proof(args):
    ...
    ax.plot(
        ...
        marker="x",
        markersize=4,
        label="Trajectoire originale (bruit)",
        zorder=1,
    )
...
def cmd_overlay(args):
    ...
    if not json_path:
        ...
        print(
            f"💡 Lancez d'abord: python run.py compress --w_error {args.w_error} --match_id {args.match_id}"
        )
        return
    ...
    if args.interactive:
        ...
        generate_static_overlay(
            CANVAS_PATH, data_dir, args.w_error, args.match_id, output_path
        )
...
def cmd_cluster(args):
    ...
    if path_float.exists():
        ...
        print(
            f"💡 Lancez d'abord: python run.py compress-batch --w_errors {args.w_error}"
        )
        return
...
Exemples:
  ...
  python run.py visu_cluster 5 --w_error 12
    ...
    parser_compress = subparsers.add_parser(
        "compress", help="Compresser les trajectoires"
    )
    parser_compress.add_argument(
        "--w_error", type=float, required=True, help="Paramètre de compression"
    )
    parser_compress.add_argument(
        "--match_id", type=str, default=None, help="ID du match (optionnel, sinon tous)"
    )

    # COMPRESS-BATCH
    parser_compress_batch = subparsers.add_parser(
        "compress-batch", help="Compresser avec plusieurs w_error"
    )
    parser_compress_batch.add_argument(
        "--w_errors",
        type=str,
        required=True,
    ...
    parser_viz.add_argument("--match_id", type=str, required=True, help="ID du match")

    # VISUALIZE-BATCH
    parser_viz_batch = subparsers.add_parser(
        "visualize-batch", help="Générer visualisations en batch"
    )
    parser_viz_batch.add_argument(
        "--w_errors",
        type=str,
        default=None,
    ...
    # ZOOM-PROOF
    parser_zoom = subparsers.add_parser("zoom-proof", help="Générer preuve de concept")
    parser_zoom.add_argument(
        "--match_id",
        type=str,
        default="3841665963",
    ...
    parser_overlay.add_argument(
        "--match_id", type=str, required=True, help="ID du match"
    )
    parser_overlay.add_argument(
        "--interactive",
        action="store_true",
    ...
    # OVERLAY-SELECT
    parser_overlay_select = subparsers.add_parser(
        "overlay-select", help="Sélection interactive w_error et match"
    )
    ...
    parser_visu_clust.add_argument(
        "--w_error",
        type=float,
        default=12.0,
