...
def experiment_robustness(csv_path: Path, output_dir: Path, 
                         player_id: int = 0, w_error: float = 12.0):
    ...
    for res in results:
        ...
        variation_pct = (variation / baseline_segments) * 100 if baseline_segments > 0 else 0
...
def generate_robustness_plot(results: list, output_dir: Path, w_error: float, match_id: str):
    ...
    fig.suptitle(f'Test de Robustesse au Bruit - Match {match_id} (w_error={w_error})', 
    ...
    for idx, res in enumerate(results):
        ...
        ax.scatter(x_noisy[::step], y_noisy[::step], 
                  c='gray', s=3, alpha=0.4, label='Trajectoire bruitée')
        ...
        xs = [seg.start.x for seg in segments] + ([segments[-1].end.x] if segments else [])
        ys = [seg.start.y for seg in segments] + ([segments[-1].end.y] if segments else [])
        ax.scatter(xs, ys, c='red', s=40, zorder=10, 
                  edgecolors='darkred', linewidth=1.5,
                  label='Segments compressés')
        
        # Configuration
        noise_label = "Aucun bruit" if sigma == 0.0 else f"Bruit σ={sigma}"
        ax.set_title(f'{noise_label} → {len(segments)} segments', 
