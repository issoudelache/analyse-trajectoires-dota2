...
class MDLCompressor:
    ...
    def _can_extend_segment(self, trajectory: Trajectory, start_idx: int, end_idx: int) -> bool:
    ...
    def compress_player_trajectory(self, trajectory: Trajectory) -> List[Segment]:
        ...
        while start_idx < len(trajectory) - 1:
            ...
            segment = Segment(
                ...
                end=trajectory[best_idx]
            )
            segments.append(segment)
            
            if self.verbose:
                num_skipped = best_idx - start_idx - 1
                print(f"  Segment {len(segments)}: [{start_idx}→{best_idx}] "
    ...
    def process_full_match(self, df: pd.DataFrame, match_id: str) -> Dict[int, List[Segment]]:
...
def compress_player_trajectory(trajectory: Trajectory, w_error: float = 2.0, 
...
def process_full_match(df: pd.DataFrame, match_id: str, w_error: float = 2.0,
                      verbose: bool = False) -> Dict[int, List[Segment]]:
