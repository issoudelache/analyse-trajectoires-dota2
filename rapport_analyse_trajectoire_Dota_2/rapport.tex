...
\subsection{Choix Technologique : Python Scientifique}

L'ensemble du projet est développé en \textbf{Python 3}, un choix motivé par la maturité de son écosystème pour le calcul scientifique et l'analyse de données. Trois bibliothèques constituent le socle technique de notre pipeline :

\begin{itemize}
    \item \textbf{NumPy} : toutes les opérations critiques (distances TRACLUS, matrices de similarité $N \times N$, compression MDL) sont vectorisées via \texttt{numpy.ndarray}, ce qui permet des gains de performance de l'ordre de $\times 100$ par rapport à des boucles Python classiques.
    \item \textbf{Pandas} : utilisé pour l'ingestion et le nettoyage des fichiers CSV bruts, chaque fichier contenant un \textit{tick} par ligne avec les coordonnées des 10 joueurs.
    \item \textbf{Matplotlib} : génération de l'ensemble des visualisations du projet, des \textit{overlays} de trajectoires sur la carte de jeu aux graphiques multi-panneaux des benchmarks expérimentaux.
\end{itemize}

Les algorithmes centraux du pipeline, notamment l'\textit{Affinity Propagation} et \textit{PrefixSpan}, ont été implémentés intégralement par nos soins (\textit{from scratch}) sans recours à des bibliothèques externes, afin de maîtriser la complexité algorithmique et d'adapter finement chaque étape à notre contexte de données e-sport.
...
Le graphique en barres (Figure \ref{fig:clustering_quality}) montre de meilleures performances relatives de K-Médoïdes et de l'Affinity Propagation par rapport à K-Means. Par exemple, K-Médoïdes affiche un score de Silhouette de $\sim 0.14$ contre $0.08$ pour K-Means. Ces valeurs restent globalement basses par rapport au seuil conventionnel de $0.5$ \cite{rousseeuw1987}. Cependant, ce résultat est attendu et documenté dans la littérature sur le clustering de trajectoires : Lee et al.~\cite{lee2007} rapportent des scores de Silhouette similaires ($< 0.2$) sur des données de trajectoires denses, car les segments spatiaux se chevauchent naturellement aux intersections et zones de transit (rivière, midlane), rendant les frontières inter-clusters intrinsèquement floues. L'important est que, malgré cette séparation géométrique limitée, la projection des centres de clusters sur la carte officielle (Figure~\ref{fig:fig3_1_clusters}) confirme que les 10 zones identifiées correspondent à des régions géographiquement distinctes et stratégiquement significatives (lanes, jungles, rivière, bases), validant ainsi la pertinence sémantique du découpage au-delà du seul critère numérique. Néanmoins, la différence relative entre algorithmes s'explique : K-Means ne peut moyenner que des coordonnées brutes euclidiennes, tandis que nos deux algorithmes avancés exploitent directement la matrice de distance experte (TRACLUS), respectant ainsi la perpendicularité et le sens des trajectoires.
...
Ce test montre que le sous-échantillonnage à $N=3000$ segments constitue un bon compromis permettant à l'AP de produire un découpage cohérent de la carte de \textit{DotA 2} sans être perturbé par le bruit. Concrètement, les 30 matchs du sous-ensemble de fouille produisent environ 7\,500 segments compressés (30 matchs $\times$ 10 joueurs $\times$ $\sim$25 segments). Les $N=3000$ segments fournis à l'AP sont obtenus par un tirage aléatoire uniforme (sans remise) parmi ces 7\,500 segments, à graine fixe (\texttt{SEED=42}) pour garantir la reproductibilité. Les exemplaires (centres de clusters) ainsi identifiés sont ensuite utilisés pour réassigner l'ensemble des segments du sous-ensemble de fouille à leur cluster le plus proche.

\subsubsection{Validation Croisée et Stabilité Bootstrap de l'AP}

Afin de vérifier que le clustering AP n'est pas un artefact du jeu de données choisi, nous avons conduit deux expériences de validation complémentaires.

\paragraph{Exp A --- Validation croisée (split 50/50).}
Les 70 matchs du corpus sont répartis en deux moitiés disjointes (35~calibration / 35~validation, permutation déterministe \texttt{SEED=42}). Sur chaque moitié, nous appliquons le pipeline complet : sous-échantillonnage de 3\,000 segments, exécution d'AP (\textit{preference}~$=-5\,000$, \textit{damping}~$=0{,}7$), puis réassignation de l'ensemble des segments du split à l'exemplaire le plus proche.

\begin{table}[H]
\centering
\begin{tabular}{lcccc}
\hline
\textbf{Split} & \textbf{Segments} & $k$ & \textbf{Silhouette} & \textbf{Davies-Bouldin} \\
\hline
Calibration (35 matchs) & 23\,155 & 10 & 0{,}075 & 1{,}85 \\
Validation (35 matchs) & 20\,060 & 10 & 0{,}091 & 2{,}01 \\
\hline
\end{tabular}
\caption{Résultats de la validation croisée calibration/validation de l'AP.}
\label{tab:cv_results}
\end{table}

Le nombre de clusters $k=10$ est identique sur les deux moitiés, et les métriques (Silhouette, Davies-Bouldin) restent du même ordre de grandeur. Ce résultat confirme que la structure à 10 clusters n'est pas un artefact de sur-apprentissage mais reflète une organisation spatiale intrinsèque des trajectoires.

\paragraph{Exp B --- Stabilité bootstrap de l'échantillonnage AP.}
Nous avons tiré 10 sous-échantillons indépendants de 3\,000 segments parmi les 18\,716 segments des 30 matchs de fouille (graines 42 à 51), puis exécuté l'AP sur chacun. Pour chaque paire de tirages, nous mesurons l'\textit{Adjusted Rand Index} (ARI) sur l'intersection des segments communs, ainsi que la distance de Hausdorff entre les ensembles d'exemplaires.

\begin{table}[H]
\centering
\begin{tabular}{lccc}
\hline
\textbf{Métrique} & \textbf{Moyenne} & $\sigma$ & \textbf{Plage} \\
\hline
Nombre de clusters $k$  & 9{,}7  & 0{,}46 & $[9,\;10]$ \\
Silhouette              & 0{,}182 & 0{,}009 & --- \\
ARI pairwise            & 0{,}372 & 0{,}056 & min = 0{,}254 \\
Hausdorff (exemplaires) & 22{,}9  & 6{,}1  & --- \\
\hline
\end{tabular}
\caption{Stabilité bootstrap de l'AP sur 10 tirages aléatoires de 3\,000 segments.}
\label{tab:bootstrap_results}
\end{table}

Le nombre de clusters est remarquablement stable ($k \in \{9, 10\}$ sur 100\% des tirages, $\mu = 9{,}7$). L'ARI moyen de 0{,}37 indique un accord modéré entre les partitions issues de sous-échantillons différents, ce qui est attendu étant donné que les segments communs ne représentent qu'une fraction de chaque tirage. Les Silhouette scores présentent un écart-type de seulement 0{,}009, confirmant la stabilité des métriques de qualité.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{img/validation_plots.png}
    \caption{Résultats de la validation méthodologique : (haut) comparaison calibration vs.\ validation, (bas) stabilité bootstrap --- distribution de $k$, matrice ARI, et distances de Hausdorff entre exemplaires.}
    \label{fig:validation_plots}
\end{figure}

\subsubsection{Scalabilité vs Qualité Sémantique : Le Choix Final}

Pour comparer équitablement les algorithmes, nous fixons $k=12$ pour tous dans un benchmark contrôlé (Figure \ref{fig:comparatif_global}). En production, l'AP détermine librement le nombre optimal de clusters ($k=10$, validé par bootstrap en section précédente). Ce benchmark expose un dilemme classique entre performance statistique et coût computationnel : le K-Médoïdes s'exécute quasi instantanément ($\sim 0.03s$) et affiche le meilleur score silhouette médian ($\sim 0.15$), là où l'Affinity Propagation (AP) voit son temps de convergence exploser ($\sim 6s$ pour $N=3000$) en raison de sa complexité quadratique $\mathcal{O}(N^2)$.

Cependant, l'excellence statistique du K-Médoïdes cache une faille géographique majeure :
\begin{itemize}
    \item \textbf{L'anomalie du doublon :} L'algorithme a tendance à placer des centres de clusters extrêmement proches (parfois à moins de 5 unités), gaspillant un groupe pour désigner la même zone (ex: l'Offlane Radiant) tout en ignorant des secteurs entiers comme la Top Lane.
    \item \textbf{La supériorité sémantique de l'AP :} Malgré sa lourdeur, l'AP produit le découpage le plus cohérent. Il parvient à diviser la carte en zones d'influence équilibrées et sans doublons, ce qui est indispensable pour l'étape suivante de fouille de motifs.
\end{itemize}

\textbf{Bilan :} Bien que le K-Médoïdes soit mathématiquement plus performant sur le score silhouette, il échoue à capturer la topologie réelle du jeu. Pour garantir l'extraction de stratégies sémantiquement valides, \textbf{nous retenons l'Affinity Propagation} sur un échantillon contrôlé de $N=3000$ segments.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{img/fig3_overview_k12.jpg}
    \caption{Benchmark comparatif à $k=12$ fixé. À gauche, le score silhouette met en avant la supériorité statistique du K-Médoïdes. À droite, l'échelle logarithmique illustre l'explosion du temps de calcul de l'AP face à la montée en charge ($N$). En production, l'AP s'exécute sans contrainte sur $k$ et converge vers $k=10$.}
    \label{fig:comparatif_global}
\end{figure}
...
Un benchmark systématique (Figure~\ref{fig:prefixspan_benchmark}) permet de quantifier l'impact de chaque paramètre sur les performances de PrefixSpan et de justifier nos choix :

\begin{itemize}
    \item \textbf{Sensibilité au support minimum (haut-gauche) :} Le temps d'exécution décroît de façon exponentielle avec le seuil de support. À $5\%$, PrefixSpan requiert $\sim 1080s$ ($\approx 18$ min) ; à $15\%$, ce temps chute à $\sim 130s$ (facteur $\times 8$) ; au-delà de $30\%$, il devient négligeable ($< 10s$). Cette décroissance s'explique par l'élagage précoce de l'arbre de recherche : un seuil plus élevé permet à PrefixSpan de couper des branches entières dès qu'un préfixe devient infrequent, réduisant drastiquement l'espace exploré. Notre seuil de $15\%$ se situe au \textit{coude} de la courbe, là où le gain de temps par point de support supplémentaire commence à stagner, ce qui en fait le point d'équilibre optimal entre richesse des motifs et temps de calcul.

    \item \textbf{Explosion combinatoire avec la longueur maximale (haut-droite) :} Le nombre de motifs croît de façon quasi exponentielle avec la longueur autorisée : $\sim 2\,000$ motifs pour une longueur maximale de 2, $\sim 45\,000$ pour 3, $\sim 130\,000$ pour 4, et $\sim 155\,000$ pour 5. Le saut de longueur 2 à 3 (facteur $\times 22$) est le plus violent, car c'est à ce stade que les combinaisons entre les 10 clusters explosent. Au-delà de 4, la croissance ralentit : les motifs longs sont rares dans nos séquences de $\sim 65$ transitions. Nous limitons l'extraction à une longueur $\le 3$, ce qui capture les rotations stratégiques à 3 zones (ex: Jungle $\rightarrow$ Mid $\rightarrow$ Rivière) tout en maintenant le volume sous $50\,000$ motifs, analysable humainement.

    \item \textbf{Scalabilité linéaire avec la taille de la base (bas-gauche) :} Le temps de calcul croît de façon quasi linéaire avec le nombre de séquences : $\sim 60s$ pour 600 séquences, $\sim 140s$ pour 1250, $\sim 205s$ pour 1800, et $\sim 290s$ pour 2500. L'absence de comportement quadratique ou exponentiel confirme que PrefixSpan conserve une complexité pratique raisonnable sur nos données. Par extrapolation linéaire, traiter l'intégralité de notre corpus (70 matchs, $\sim 700$ séquences) à $15\%$ de support coûterait environ $70s$, ce qui reste parfaitement viable. Notre échantillon de 30 matchs ($\sim 300$ séquences) se situe donc bien en deçà de toute limite computationnelle.

    \item \textbf{Empreinte mémoire (bas-droite) :} La consommation mémoire présente un profil de décroissance encore plus brutal que le temps : $\sim 22$~Mo à $10\%$, puis une chute abrupte sous $1$~Mo dès $30\%$. Ce comportement de \textit{cliff} s'explique par la taille de la base de données projetées que PrefixSpan maintient en mémoire : à faible support, des milliers de préfixes infrequents sont conservés, gonflant la mémoire. À $15\%$, l'empreinte est d'environ $5$--$10$~Mo, ce qui est compatible avec un déploiement sur n'importe quelle machine standard sans contrainte matérielle.
\end{itemize}

\textbf{Synthèse du benchmark :} Ces quatre courbes convergent vers une même conclusion : le paramétrage retenu ($\text{support} = 15\%$, $\text{longueur} \le 3$, $N \approx 300$ séquences) se situe systématiquement dans une zone de compromis favorable, loin des régimes d'explosion combinatoire ou de saturation mémoire. PrefixSpan, sous ces conditions, s'exécute en $\sim 2$ minutes avec une empreinte mémoire négligeable, ce qui valide sa viabilité comme étape finale de notre pipeline d'analyse.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{img/prefixspan_benchmark.jpg}
    \caption{Benchmark de PrefixSpan selon quatre axes. \textit{Haut-gauche} : décroissance exponentielle du temps d'exécution avec le support minimum ; le coude à $15\%$ justifie notre seuil. \textit{Haut-droite} : explosion combinatoire du nombre de motifs avec la longueur maximale ; le saut entre 2 et 3 (facteur $\times 22$) est le plus critique. \textit{Bas-gauche} : scalabilité quasi linéaire avec la taille de la base de séquences. \textit{Bas-droite} : empreinte mémoire en chute libre au-delà de $20\%$ de support.}
    \label{fig:prefixspan_benchmark}
\end{figure}
...
\textit{Mining Sequential Patterns}.
In Proceedings of the 11th International Conference on Data Engineering (ICDE~'95), pages 3--14, 1995.
...
In Proceedings of the 2nd International Conference on Knowledge Discovery and Data Mining (KDD~'96), pages 226--231, 1996.

\bibitem{opendota}
OpenDota.
\textit{OpenDota API Documentation}.
\url{https://docs.opendota.com/}, consulté en 2025.

\bibitem{han2011}
J. Han, J. Pei, and M. Kamber.
